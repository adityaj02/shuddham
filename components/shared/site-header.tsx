...
export const SiteHeader = () => {
  ...
  const handleLogout = async () => {
    ...
    // Use router.push for a smooth client-side navigation (no full reload)
    router.push("/");
    router.refresh();
  };
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-[60] flex md:hidden animate-in fade-in duration-300">
          ...
          <div className="relative w-64 max-w-[80vw] bg-surface-container-low h-full shadow-2xl flex flex-col pt-6 font-body animate-in slide-in-from-left duration-300">
            {/* Sidebar header — shows user info when logged in */}
            <div className="flex items-center gap-3 px-6 pb-6 border-b border-primary/10">
              {user ? (
                <>
                  ...
                  <div className="flex-1 min-w-0">
                    {displayName && (
                      <p className="font-bold text-primary text-sm truncate">{displayName}</p>
                    )}
              ...
              <button onClick={() => setIsSidebarOpen(false)} className="text-secondary hover:text-primary transition-colors ml-auto shrink-0">
            ...
            <div className="p-6 border-t border-primary/10 bg-primary/5">
              {user ? (
                <button onClick={handleLogout} className="w-full py-3 bg-white border border-primary/10 text-primary text-xs uppercase tracking-widest font-bold rounded-xl shadow-sm hover:bg-primary/5 transition-colors">
                  Logout
                </button>
              ) : (
          ...
          {user ? (
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <div className={`${user.user_metadata?.avatar_url ? 'hidden' : ''} flex items-center justify-center w-full h-full text-primary font-bold text-xs uppercase`}>
                  {user.email?.[0] || <span className="material-symbols-outlined text-xl">person</span>}
                </div>
              </Link>
              <button
